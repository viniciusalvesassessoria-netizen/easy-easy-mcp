import { getSupabase } from './supabase-client.js';
import type { ToolContext } from './types.js';
import { ok, requireUserId, toolError } from './tool-shared.js';

export async function setEnabled(args: { enabled: boolean }, context: ToolContext) {
  try {
    const userId = requireUserId(context.authenticatedUserId);
    const { error } = await getSupabase().from('user_preferences').upsert(
      { user_id: userId, enabled: args.enabled, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
    if (error) throw error;
    return ok({ success: true, enabled: args.enabled });
  } catch (error) { return toolError('set_enabled', error); }
}
